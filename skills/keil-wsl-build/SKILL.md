...
description: Build and verify Keil uVision/UV4 projects from WSL using the Windows command-line compiler at the exact clean WSL Git HEAD. Use when a user asks to compile, rebuild, or validate one or more .uvprojx targets with Keil from WSL, especially when WSL and Windows use separate clones or Git worktrees.
---

# Keil WSL Build

Run Keil through its Windows CLI without opening the GUI. The bundled script validates that the WSL and Windows clones identify the same remote, transfers the exact WSL `HEAD` when necessary, and builds from a disposable Windows Git worktree.
...
5. Build all tracked `*.uvprojx` files and all declared targets unless the user narrows the request:
...
7. Report the exact commit, every target's error and warning counts, and the summary/log directory.

## Preconditions and interpretation

- Require a clean WSL worktree, including staged, unstaged, untracked, and submodule state. If it is dirty, stop and explain that a clean committed `HEAD` is required for an exact cross-clone build.
...
- The script may fetch objects into the Windows clone or transfer `HEAD` with a temporary bundle. It builds only in a disposable Windows worktree and removes firmware artifacts afterward.
