...
**Profile**（`profiles/`）定义设备协议：寄存器/线圈地址、名称、读写权限、枚举、单位、串口默认值、默认 State 字段等。串口元数据 `serial.bytesize`、`serial.parity`、`serial.stopbits` 分别定义数据位、校验（`N`/`E`/`O`）和停止位；缺省时兼容旧 Profile 的 8N1。
